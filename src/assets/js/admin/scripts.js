...
/******************************************************************
 * 강의결과보고서 프린트 자동화 스크립트
 * (.printReportPrintBtn 클릭 → print-only 자동 부여 → 인쇄 → 복구)
 ******************************************************************/
(function () {
  document.addEventListener("DOMContentLoaded", function () {
    const printBtn = document.querySelector(".printReportPrintBtn");
    if (!printBtn) return;

    printBtn.addEventListener("click", function () {
      // 인쇄 대상 page들
      const printTargets = document.querySelectorAll(".print-report-page");
      const PRINT_CLASS = "print-only";

      // 원래 print-only였던 요소 저장 (복구용)
      const originallyPrintOnly = new Set();
      document.querySelectorAll("." + PRINT_CLASS).forEach(el => {
        originallyPrintOnly.add(el);
      });

      // 1) 출력 대상에 print-only 클래스 부여
      printTargets.forEach(el => {
